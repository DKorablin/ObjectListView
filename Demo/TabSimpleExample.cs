	...
	public partial class TabSimpleExample : OlvDemoTab
	{
		...
		private void SetupMouseMoveTracking()
		{
			...
			this.olvSimple.MouseHover += delegate (Object sender, EventArgs e)
			{
				...
				OLVListItem item = this.olvSimple.GetItemAt(pt.X, pt.Y, out OLVColumn column);
		...
		private void SetupDragToGroup()
		{
			...
			this.olvSimple.ModelDropped += delegate (Object sender, ModelDropEventArgs e)
			{
				...
				if(!(e.TargetModel is Person target))
					return;
		...
		private void buttonListAnimation_Click(Object sender, EventArgs e)
		{
			...
			Sprite image = new ImageSprite(Resource.largestar);
			...
			{
				Opacity = 0.0f,
				FixedLocation = Locators.SpriteAligned(Corner.MiddleCenter)
			};
		...
		private void buttonRowAnimation_Click(Object sender, EventArgs e)
		{
			...
			if(modelObject == null)
			{
				this.Coordinator.ShowMessage("There aren't any rows to be animated");
				return;
			}
		...
		private void AddStarAnimation(Animation animation, Int32 start, Single opacity)
		{
			Sprite sprite = new ImageSprite(Resource.star32)
			{
				Opacity = opacity
			};
		...
		private void buttonCellAnimation_Click(Object sender, EventArgs e)
		{
			...
			{
				this.Coordinator.ShowMessage("There aren't any cells to be animated");
				return;
			}
