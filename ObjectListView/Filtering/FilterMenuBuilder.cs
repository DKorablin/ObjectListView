...
namespace BrightIdeasSoftware
{
	...
	public class FilterMenuBuilder
	{
		...
		/// </summary>
		public Boolean TreatNullAsDataValue { get; set; } = true;
		...
		/// </summary>
		/// <remarks>The default value is 10,000. This should be perfectly acceptable for almost all lists.</remarks>
		public Int32 MaxObjectsToConsider { get; set; } = 10000;

		#endregion

		/// <summary>Create a Filter menu on the given tool tip for the given column in the given ObjectListView.</summary>
		...
		{
			_ = strip ?? throw new ArgumentNullException(nameof(strip));
		...
		/// <summary>Create a collection of clusters that should be presented to the user</summary>
		...
		/// <summary>Order the given list of clusters in the manner in which they should be presented to the user.</summary>
		/// <param name="strategy"></param>
		/// <param name="clusters"></param>
		virtual protected void SortClusters(IClusteringStrategy strategy, List<ICluster> clusters)
			=> clusters.Sort();

		/// <summary>Do the work of making a menu that shows the clusters to the users</summary>
		/// <param name="column"></param>
		/// <param name="clusters"></param>
		/// <returns></returns>
		virtual protected ToolStripMenuItem CreateFilteringMenuItem(OLVColumn column, List<ICluster> clusters)
		{
			...
			{
				Int32 checkedCount = checkedList.CheckedItems.Count;
				if(checkedCount == 0)
			...
			ToolStripMenuItem clearAll = new ToolStripMenuItem(CLEAR_ALL_FILTERS_LABEL, ClearFilteringImage,
				(sender, args) => this.ClearAllFilters(column));

			ToolStripMenuItem apply = new ToolStripMenuItem(APPLY_LABEL, FilteringImage,
				(sender, args) => this.EnactFilter(checkedList, column));
		...
		/// <summary>Handle a user-generated ItemCheck event</summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		...
		virtual protected void HandleSelectAllItem(ItemCheckEventArgs e, ToolStripCheckedListBox checkedList, Int32 selectAllIndex)
		{
			...
			Int32 count = checkedList.CheckedItems.Count;
		...
		/// <summary>Clear all the filters that are applied to the given column</summary>
		...
		/// <summary>Apply the selected values from the given list as a filter on the given column</summary>
