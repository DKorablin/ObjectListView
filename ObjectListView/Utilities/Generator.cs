...
{
	/// <summary>
	/// An object that implements the IGenerator interface provides the ability to dynamically create columns
	...
	public interface IGenerator
	{
		...
		void GenerateAndReplaceColumns(ObjectListView olv, Type type, Boolean allProperties);
	...
	public class Generator : IGenerator
	{
		...
		/// <summary>Gets or sets the actual generator used by the static convenience methods.</summary>
		...
		{
			get => Generator._instance ?? (Generator._instance = new Generator());
			set => Generator._instance = value;
		}
		private static IGenerator _instance;

		/// <summary>
		/// Replace all columns of the given ObjectListView with columns generated from the first member of the given enumerable.
		/// If the enumerable is empty or null, the ObjectListView will be cleared.
		/// </summary>
		/// <param name="olv">The ObjectListView to modify</param>
		...
		/// <param name="enumerable">The collection whose first element will be used to generate columns.</param>
		/// <param name="allProperties">Will columns be generated for properties that are not marked with [OLVColumn].</param>
		static public void GenerateColumns(ObjectListView olv, IEnumerable enumerable, Boolean allProperties)
		{
		...
		/// <param name="type">The model type whose attributes will be considered.</param>
		/// <param name="allProperties">Will columns be generated for properties that are not marked with [OLVColumn].</param>
		static public void GenerateColumns(ObjectListView olv, Type type, Boolean allProperties)
		...
		/// <param name="type">The model type whose attributes will be considered.</param>
		/// <param name="allProperties">Will columns be generated for properties that are not marked with [OLVColumn].</param>
		public virtual void GenerateAndReplaceColumns(ObjectListView olv, Type type, Boolean allProperties)
		{
		...
		public virtual IList<OLVColumn> GenerateColumns(Type type, Boolean allProperties)
		{
			...
			Int32 countPositiveDisplayIndex = 0;
			foreach(OLVColumn col in columns)
				if(col.DisplayIndex >= 0)
					countPositiveDisplayIndex += 1;

			// Give columns that don't have a DisplayIndex an incremental index
			Int32 columnIndex = countPositiveDisplayIndex;
			foreach(OLVColumn col in columns)
				if(col.DisplayIndex < 0)
					col.DisplayIndex = (columnIndex++);

			columns.Sort((OLVColumn x, OLVColumn y) => x.DisplayIndex.CompareTo(y.DisplayIndex));
		...
		protected virtual OLVColumn MakeColumn(String aspectName, String title, Boolean editable, Type propertyType, OLVColumnAttribute attr)
		{
			...
			if(attr.TriStateCheckBoxesSet)
				column.TriStateCheckBoxes = attr.TriStateCheckBoxes;
			if(attr.UseInitialLetterForGroupSet)
			...
			if(attr.UseFilteringSet)
				column.UseFiltering = attr.UseFiltering;
			if(attr.AutoCompleteEditorSet)
				column.AutoCompleteEditor = attr.AutoCompleteEditor;
			if(attr.AutoCompleteEditorModeSet)
				column.AutoCompleteEditorMode = attr.AutoCompleteEditorMode;
			if(attr.WordWrapSet)
				column.WordWrap = attr.WordWrap;
			return column;
		}
		...
		/// <summary>Generate methods so that reflection is not needed.</summary>
		/// <param name="olv"></param>
		/// <param name="type"></param>
