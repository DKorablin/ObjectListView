...
namespace BrightIdeasSoftware
{
	...
	{
		/// <summary>Create a VirtualObjectListView</summary>
		...
		{// Virtual lists need Vista and a grouping strategy to show groups
			get => (ObjectListView.IsVistaOrLater && this.GroupingStrategy != null);
		}
		...
		/// </summary>
		/// <remarks>This property is an implementation detail and should not be altered.</remarks>
		protected internal Boolean CheckedObjectsMustStillExistInList { get; set; } = true;
		...
		/// <summary>Gets or sets the strategy that will be used to create groups</summary>
		/// <remarks>This must be provided for a virtual list to show groups.</remarks>
		[Browsable(false),
		DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		/// <remarks>This is only possible if our underlying data source supports filtering.</remarks>
		public override Boolean IsFiltering
		{
			get => base.IsFiltering && (this.VirtualListDataSource is IFilterableDataSource);
		}
		...
		/// <summary>This delegate is used to fetch a rowObject, given it's index within the list</summary>
		...
		[DefaultValue(true)]
		override public Boolean ShowGroups
		{
			get => ObjectListView.IsVistaOrLater && this._showGroups;// Pre-Vista, virtual lists cannot show groups
			set
			{
		...
		/// <summary>Get/set the data source that is behind this virtual list</summary>
		...
		/// <summary>Gets or sets the number of rows in this virtual list.</summary>
		/// <remarks>
		...
		[Browsable(false)]
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		protected new virtual Int32 VirtualListSize
		{
		...
		public override Object GetModelObject(Int32 index)
			=> this.VirtualListDataSource != null && index >= 0 && index < this.GetItemCount()
		...
		public override Int32 IndexOf(Object modelObject)
			=> this.VirtualListDataSource == null || modelObject == null
				? -1
				: this.VirtualListDataSource.GetObjectIndex(modelObject);
		...
		public override OLVListItem ModelToItem(Object modelObject)
		{
			...
			Int32 index = this.VirtualListDataSource.GetObjectIndex(modelObject);
		...
		/// </remarks>
		public virtual void EnsureNthGroupVisible(Int32 groupIndex)
		{
			if(!this.ShowGroups)
				return;
			...
			} else
			{
				...
				Int32 lastItemInGroup = this.GroupingStrategy.GetGroupMember(previousGroup, previousGroup.VirtualItemCount - 1);
		...
		/// <summary>Inserts the given collection of model objects to this control at hte given location</summary>
		...
		/// <summary>Select the row that is displaying the given model Object. All other rows are deselected.</summary>
		/// <param name="modelObject">Model Object to select</param>
		/// <param name="setFocus">Should the Object be focused as well?</param>
		public override void SelectObject(Object modelObject, Boolean setFocus)
		{
			// Without a data source, we can't do this.
			if(this.VirtualListDataSource == null)
				return;
		...
		public override void SetObjects(IEnumerable collection, Boolean preserveState)
		{
			if(this.InvokeRequired)
			{
		...
		protected override CheckState? GetCheckState(Object modelObject)
		{
			...
			return modelObject != null && this.CheckStateMap.TryGetValue(modelObject, out CheckState state)
				? state
		...
		public override void ClearCachedInfo()
			=> this._lastRetrieveVirtualItemIndex = -1;

		/// <summary>Do the work of creating groups for this control</summary>
		...
		/// <summary>Do the plumbing to disable groups on a virtual list</summary>
		protected void DisableVirtualGroups()
		{
			...
			_ = NativeMethods.SendMessage(this.Handle, LVM_ENABLEGROUPVIEW, 0, 0);
			//System.Diagnostics.Debug.WriteLine(x);

			const Int32 LVM_SETOWNERDATACALLBACK = 0x10BB;
			_ = NativeMethods.SendMessage(this.Handle, LVM_SETOWNERDATACALLBACK, 0, 0);
			//System.Diagnostics.Debug.WriteLine(x2);
		}

		/// <summary>Do the plumbing to enable groups on a virtual list</summary>
		protected void EnableVirtualGroups()
		{

			// We need to implement the IOwnerDataCallback interface
			if(this._ownerDataCallbackImpl == null)
				this._ownerDataCallbackImpl = new OwnerDataCallbackImpl(this);

			const Int32 LVM_SETOWNERDATACALLBACK = 0x10BB;
			IntPtr ptr = Marshal.GetComInterfaceForObject(this._ownerDataCallbackImpl, typeof(IOwnerDataCallback));
			_ = NativeMethods.SendMessage(this.Handle, LVM_SETOWNERDATACALLBACK, ptr, 0);
			//System.Diagnostics.Debug.WriteLine(x);
			Marshal.Release(ptr);

			const Int32 LVM_ENABLEGROUPVIEW = 0x1000 + 157;
			_ = NativeMethods.SendMessage(this.Handle, LVM_ENABLEGROUPVIEW, 1, 0);
			//System.Diagnostics.Debug.WriteLine(x);
		}
		...
		/// <returns></returns>
		public override Int32 GetDisplayOrderOfItemIndex(Int32 itemIndex)
		{
			if(!this.ShowGroups)
				return itemIndex;

			Int32 groupIndex = this.GroupingStrategy.GetGroup(itemIndex);
			Int32 displayIndex = 0;
			for(Int32 i = 0; i < groupIndex; i++)
		...
		/// <param name="n"></param>
		/// <returns></returns>
		public override OLVListItem GetNthItemInDisplayOrder(Int32 n)
		{
		...
		public override OLVListItem GetNextItem(OLVListItem itemToFind)
		{
			...
			Int32 groupIndex = this.GroupingStrategy.GetGroup(itemToFind.Index);
			Int32 indexWithinGroup = this.GroupingStrategy.GetIndexWithinGroup(this.OLVGroups[groupIndex], itemToFind.Index);
		...
		protected override IList<OLVGroup> MakeGroups(GroupingParameters parms)
			=> this.GroupingStrategy == null
				? new List<OLVGroup>()
				: this.GroupingStrategy.GetGroups(parms);
		...
		public virtual OLVListItem MakeListViewItem(Int32 itemIndex)
		{
		...
		/// <summary>On virtual lists, this cannot work.</summary>
		...
		/// <returns>The check state that was recorded and that should be used to update the control.</returns>
		...
		/// <summary>Refresh the given item in the list</summary>
		...
		/// <summary>Change the size of the list</summary>
		/// <param name="newSize"></param>
		protected virtual void SetVirtualListSize(Int32 newSize)
		{
			if(newSize < 0 || this.VirtualListSize == newSize)
				return;

			Int32 oldSize = this.VirtualListSize;
		...
		/// <remarks>
		/// <para>
		/// This method separates the 'objects' instance variable from its source, so that any AddObject/RemoveObject
		/// calls will modify our collection and not the original collection.
		/// </para>
		/// <para>VirtualObjectListViews always own their collections, so this is a no-op.</para>
		/// </remarks>
		protected override void TakeOwnershipOfObjects()
		{
		}

		/// <summary>Change the state of the control to reflect changes in filtering</summary>
		protected override void UpdateFiltering()
		{
			if(!(this.VirtualListDataSource is IFilterableDataSource filterable))
				return;
		...
		/// <summary>Change the size of the virtual list so that it matches its data source</summary>
		...
		/// <summary>Handle the CacheVirtualItems event</summary>
		...
		/// <summary>Handle a RetrieveVirtualItem</summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		protected virtual void HandleRetrieveVirtualItem(Object sender, RetrieveVirtualItemEventArgs e)
		{
			...
			if(this._lastRetrieveVirtualItemIndex != e.ItemIndex)
			{
				this._lastRetrieveVirtualItemIndex = e.ItemIndex;
				this._lastRetrieveVirtualItem = this.MakeListViewItem(e.ItemIndex);
			}
			e.Item = this._lastRetrieveVirtualItem;
		}

		/// <summary>Handle the SearchForVirtualList event, which is called when the user types into a virtual list</summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		protected virtual void HandleSearchForVirtualItem(Object sender, SearchForVirtualItemEventArgs e)
		{
			...
			Int32 start = Math.Min(e.StartIndex, this.VirtualListDataSource.GetObjectCount() - 1);
			...
			// Do the search
			Int32 i = this.FindMatchingRow(args.StringToFind, args.StartSearchFrom, e.Direction);
		...
		/// <summary>Handle the VirtualItemsSelectionRangeChanged event, which is called "when the selection state of a range of items has changed"</summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		...
		protected override Int32 FindMatchInRange(String text, Int32 first, Int32 last, OLVColumn column)
			=> this.VirtualListDataSource.SearchText(text, first, last, column);
